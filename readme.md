...
## 5. Применение миграций и тестовые данные

После того как база данных поднята в Docker и `.env` заполнен:

```bash
python manage.py makemigrations
python manage.py migrate
python manage.py createsuperuser   # свой админ-аккаунт (по желанию)
python manage.py seed_demo_data    # создаст 3 тестовых пользователя и по проекту от каждого
python manage.py runserver
```
...
у каждого есть свой проект, плюс немного избранного/участия для проверки фильтров на странице «Участники».

Реализован Вариант №1 (Избранное + фильтры участников). Папки `templates_var2` и `templates_var3` удалены за ненадобностью, `TASK_VERSION` зафиксирован на `1`.

## 6. Права администратора

- Любой проект можно отредактировать/удалить владельцу или пользователю с `is_staff=True` — кнопка «Удалить проект» появляется на странице проекта.
- Создание, блокировка (`is_active`), удаление аккаунтов и смена пароля любого пользователя выполняются через стандартную Django-админку `/admin/` (зайти под `createsuperuser`). В списке пользователей есть массовые действия «Заблокировать/Разблокировать выбранных пользователей», а на странице пользователя — стандартная ссылка смены пароля.
...
Покрыты: регистрация/логин (в т.ч. дубликат email, неверный пароль), редактирование профиля, фильтры на странице участников, CRUD проектов и права доступа (владелец/не владелец/staff), избранное, участие в проекте, завершение проекта.
